void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='b') showBezier=!showBezier; 
    if(key=='c') ; 
    if(key=='d') ;  
    if(key=='e') ;
    if(key=='f') fill=!fill;
    ...
    if(key=='u') ;
    if(key=='v') showControlVectors=!showControlVectors; 
    ...
    if(key=='A') ;
    if(key=='B') showBezierConstruction=!showBezierConstruction;  
    ...
    if(key=='L') ;
    if(key=='M') ; 
    if(key=='N') showNevilleConstruction=!showNevilleConstruction;
    if(key=='O') ;
    if(key=='P') ;
    if(key=='Q') ;
...
void mousePressed()   // executed when the mouse is pressed
  {
  PNT M=Mouse();
  // Sets P to refer to the vertex closest to mouse M
  //P=A; // initial guess (assigns pointer, does not make a copy)
  //float bestDist=dist(M,A); // best distance so far
  //if(dist(M,B)<bestDist) {bestDist=dist(M,B); P=B;} 
  //if(dist(M,C)<bestDist) {bestDist=dist(M,C); P=C;} 
  //if(dist(M,D)<bestDist) {bestDist=dist(M,D); P=D;} 
  P=Point[0];
